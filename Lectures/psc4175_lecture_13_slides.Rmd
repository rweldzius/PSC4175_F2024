...
- **E**rror ( $\varepsilon$ ): actual outcome ( $Y_i$ ) - predicted outcome ( $\hat{Y}_i$ )
  ...
  - The "distance" between the data and the model: $Y_i - \hat{Y}_i$
  
--

- **S**quared: $\varepsilon^2$

  1. Makes all values positive: $(Y_i - \hat{Y}_i)^2$
  
  2. .blue[Exaggerates] the presence of larger errors
  
--

- **M**ean: average these squared errors: $\frac{1}{n}\sum_{i = 1}^n(Y_i - \hat{Y}_i)^2$
